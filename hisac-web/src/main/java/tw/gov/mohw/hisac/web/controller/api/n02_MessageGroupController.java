...
public class n02_MessageGroupController extends BaseController {

	@Autowired
	private MessageGroupService messageGroupService;

	private String targetControllerName = "not";
	private String targetActionName = "n02";
	 ...
	 * @return 警訊發佈群組資料
	 */
	@RequestMapping(value = "/n02/query", method = RequestMethod.POST)
		...
		if (menuService.getReadPermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			...
			if (messageGroups != null)
				for (MessageGroup messageGroup : messageGroups) {
					...
					sn_json.put("IsEnable", messageGroup.getIsEnable());
					sn_json.put("CreateId", messageGroup.getCreateId());
					sn_json.put("CreateTime", WebDatetime.toString(messageGroup.getCreateTime()));
					sn_json.put("ModifyId", messageGroup.getModifyId());
					sn_json.put("ModifyTime", WebDatetime.toString(messageGroup.getModifyTime()));
	 ...
	 * @return 警訊發佈群組資料
	 */
	@RequestMapping(value = "/n02/query/id", method = RequestMethod.POST)
	public String QueryById(Locale locale, HttpServletRequest request, Model model, @RequestBody String json) {
		JSONArray sn_array = new JSONArray();
		if (menuService.getReadPermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			JSONObject obj = new JSONObject(json);
			long id = obj.isNull("Id") == true ? 0 : obj.getLong("Id");
			MessageGroup messageGroup = messageGroupService.getById(id);
			JSONObject sn_json = new JSONObject();
			sn_json.put("Id", messageGroup.getId());
			sn_json.put("Name", messageGroup.getName());
			sn_json.put("IsEnable", messageGroup.getIsEnable());
	 ...
	 * @return 是否新增成功
	 */
	@RequestMapping(value = "/n02/create", method = RequestMethod.POST)
	public @ResponseBody String Create(Locale locale, HttpServletRequest request, @RequestBody String json) {
		...
		if (menuService.getCreatePermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			...
			if (messageGroupService.isNameExist(name)) {
				...
				MessageGroup messageGroup = messageGroupService.insert(getBaseMemberId(), json);
	 ...
	 * @return 是否更新成功
	 */
	@RequestMapping(value = "/n02/update", method = RequestMethod.POST)
	public @ResponseBody String Update(Locale locale, HttpServletRequest request, @RequestBody String json) {
		...
		if (menuService.getUpdatePermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			...
			if (!messageGroupService.isExist(id)) {
				...
				MessageGroup messageGroup = messageGroupService.update(getBaseMemberId(), json);
				if (messageGroup != null) {
	 ...
	 * @return 是否刪除成功
	 */
	@RequestMapping(value = "/n02/delete", method = RequestMethod.POST)
	public @ResponseBody String DeleteById(Locale locale, HttpServletRequest request, @RequestBody String json) {
		...
		if (menuService.getDeletePermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			...
			if (!messageGroupService.isExist(id)) {
				...
				if (messageGroupService.delete(id)) {
