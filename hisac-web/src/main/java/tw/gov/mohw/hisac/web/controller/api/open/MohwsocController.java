...
public class MohwsocController {
	...
	private String baseControllerName = "MowhsocAPI";
	 ...
	 * @return 是否新增成功
	 */
	@RequestMapping(value = "/mohwsoc", method = RequestMethod.POST)
	public String CreateMOHWSOC(Locale locale, HttpServletRequest request, HttpServletResponse response, Model model, @RequestBody String json) {
		...
		if (WebNet.isAllowIp(resourceMessageService.getMessageValue("mohwsocAllowIp"), WebNet.getIpAddr(request))) {
			JSONObject obj = new JSONObject(json);
			String incidentId = obj.isNull("IncidentId") == true ? null : obj.getString("IncidentId");
//			incidentId = "mohwsoc-" + incidentId;
			obj.put("SourceCode", "MOHWSOC");
			...
			if (infor != null) {
				...
				if (infor.getStatus() == 1) {
					...
					if (informationExchange != null) {
						...
						responseJson.put("debug-body", obj);
	 ...
	 * @return 是否成功
	 */
	@RequestMapping(value = "/mohwsoc/attach/upload", method = RequestMethod.POST)
	public String UploadAttach(Locale locale, HttpServletRequest request, HttpServletResponse response, Model model, @RequestBody String json) {
		...
		if (WebNet.isAllowIp(resourceMessageService.getMessageValue("mohwsocAllowIp"), WebNet.getIpAddr(request))) {
			...
			informationExchangeId = "mohwsoc-" + informationExchangeId;
	 ...
	 *            附件Id
	 */
	@RequestMapping(value = "/mohwsoc/attach/download/{informationExchangeId}/{id}", method = RequestMethod.GET)
