...
public class s04_AlertTypeController extends BaseController {
	@Autowired
	private AlertTypeService alertTypeService;

	private String targetControllerName = "sys";
	private String targetActionName = "s04";
	 ...
	 * @return Alert Type資料
	 */
	@RequestMapping(value = "/s04/query", method = RequestMethod.POST)
	public String Query(Locale locale, HttpServletRequest request, Model model, @RequestBody String json) {
		JSONObject listjson = new JSONObject();
		if (menuService.getReadPermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			JSONArray sn_array = new JSONArray();
			listjson.put("total", alertTypeService.getListSize(json));
			List<AlertType> alerts = alertTypeService.getList(json);
			if (alerts != null) {
				for (AlertType alert : alerts) {
					JSONObject sn_json = new JSONObject();
					sn_json.put("Id", alert.getId());
					sn_json.put("Code", alert.getCode());
					sn_json.put("Name", alert.getName());
					sn_json.put("IsEnable", alert.getIsEnable());
					sn_json.put("Sort", alert.getSort());
	 ...
	 * @return Alert Type資料
	 */
	@RequestMapping(value = "/s04/query/id", method = RequestMethod.POST)
	public String QueryById(Locale locale, HttpServletRequest request, Model model, @RequestBody String json) {
		JSONArray sn_array = new JSONArray();
		if (menuService.getReadPermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			JSONObject obj = new JSONObject(json);
			long id = obj.isNull("Id") == true ? 0 : obj.getLong("Id");
			AlertType alertType = alertTypeService.get(id);
			JSONObject sn_json = new JSONObject();
			sn_json.put("Id", alertType.getId());
			sn_json.put("Code", alertType.getCode());
			sn_json.put("Name", alertType.getName());
			sn_json.put("IsEnable", alertType.getIsEnable());
			sn_json.put("Sort", alertType.getSort());
			sn_array.put(sn_json);
			systemLogService.insert(baseControllerName, baseActionName, json, SystemLogVariable.Action.Read, SystemLogVariable.Status.Success, getBaseIpAddress(), getBaseMemberAccount());
		} else {
			systemLogService.insert(baseControllerName, baseActionName, json, SystemLogVariable.Action.Read, SystemLogVariable.Status.Deny, getBaseIpAddress(), getBaseMemberAccount());
	 ...
	 * @return 是否新增成功
	 */
	@RequestMapping(value = "/s04/create", method = RequestMethod.POST)
	public @ResponseBody String Create(Locale locale, HttpServletRequest request, @RequestBody String json) {
		...
		if (menuService.getCreatePermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			...
			if (alertTypeService.isCodeExist(code)) {
				...
				AlertType alertType = alertTypeService.insert(getBaseMemberId(), json);
				if (alertType != null) {
	 ...
	 * @return 是否更新成功
	 */
	@RequestMapping(value = "/s04/update", method = RequestMethod.POST)
	public @ResponseBody String Update(Locale locale, HttpServletRequest request, @RequestBody String json) {
		...
		if (menuService.getUpdatePermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			...
			if (!alertTypeService.isExist(id)) {
				...
				AlertType alertType = alertTypeService.update(getBaseMemberId(), json);
				if (alertType != null) {
	 ...
	 * @return 是否刪除成功
	 */
	@RequestMapping(value = "/s04/delete", method = RequestMethod.POST)
