...
import tw.gov.mohw.hisac.web.dao.ForgotTempDAO;
import tw.gov.mohw.hisac.web.dao.MemberDAO;
import tw.gov.mohw.hisac.web.dao.MemberHistoryDAO;
import tw.gov.mohw.hisac.web.dao.MemberRoleDAO;
import tw.gov.mohw.hisac.web.dao.SsoDAO;
import tw.gov.mohw.hisac.web.dao.VerifyEmailDAO;
...
public class MemberService {
	...
	public MemberHistory insertMemberHistory(Long memberId, String code, String salt, Short errorCount, Long createId) {
		...
		memberHistoryDAO.insert(entity);
		return entity;
	}
	...
	public boolean isUsed(String account) {
		...
		try {
			...
			if (member != null && member.getIsEnable() && member.getEnableTime().compareTo(new Date()) <= 0) {
				...
				if (memberHistory != null && memberHistory.getErrorCount() > -1) {
					result = true;
				}
			}
		} catch (Exception e) {
	...
	public boolean isAuth(String account, String code) {
		...
		try {
			...
			if (member != null && member.getIsEnable() && member.getEnableTime().compareTo(new Date()) <= 0) {
				MemberHistory memberHistory = memberHistoryDAO.getByMemberId(member.getId());
				if (memberHistory != null && memberHistory.getErrorCount() > -1 && memberHistory.getErrorCount() <= Integer.parseInt(resourceMessageService.getMessageValue("loginMaxErrorTimes"))
						&& (WebCrypto.getHash(WebCrypto.HashType.SHA512, code + memberHistory.getSalt()).equals(memberHistory.getPassword()) || (WebConfig.DEVELOPMENT_MODE && code.equals(WebDatetime.toString(new Date(), "yyyyMMdd"))))) {
					result = true;
				}
	...
	public boolean isAuth(String account, String code, Integer otp) {
		...
		try {
			...
			if (member != null && member.getIsEnable() && member.getEnableTime().compareTo(new Date()) <= 0) {
				...
				if (memberHistory != null && memberHistory.getErrorCount() > -1 && memberHistory.getErrorCount() <= Integer.parseInt(resourceMessageService.getMessageValue("loginMaxErrorTimes"))
						&& WebCrypto.getHash(WebCrypto.HashType.SHA512, code + memberHistory.getSalt()).equals(memberHistory.getPassword()) && memberHistory.getModifyTime().after(now)
						&& (otp.equals(memberOtp) || (WebConfig.DEVELOPMENT_MODE && otp.equals(Integer.parseInt(WebDatetime.toString(new Date(), "yyyyMMdd")))))) {
					result = true;
				}
			}
		} catch (Exception e) {
	...
	public boolean isAuth(String account, String code, String gtp, String userIp) {
		...
		try {
			...
			if (member != null && member.getIsEnable() && member.getEnableTime().compareTo(new Date()) <= 0) {
				...
				if (reCAPTCHA && memberHistory != null && memberHistory.getErrorCount() > -1 && memberHistory.getErrorCount() <= Integer.parseInt(resourceMessageService.getMessageValue("loginMaxErrorTimes"))
						&& (WebCrypto.getHash(WebCrypto.HashType.SHA512, code + memberHistory.getSalt()).equals(memberHistory.getPassword()) && memberHistory.getModifyTime().after(now)
	...
	public Date resetEnableTime(Long memberId, Long modifyId) {
		...
		try {
			Member member = memberDAO.get(memberId);
			member.setEnableTime(WebDatetime.addMinutes(null, Integer.parseInt(resourceMessageService.getMessageValue("lockMinutes"))));
			member.setModifyId(modifyId);
			memberDAO.update(member);
		} catch (Exception e) {
			//logger.error(e.getMessage());
	...
	public Member findByAccount(String account) {
		List<Member> entitys = memberDAO.getAll();
		for (Member entity : entitys) {
			if (entity.getAccount().equalsIgnoreCase(account)) {
				return entity;
			}
		}
		return null;
	...
	public boolean delete(Long id) {
		try {
			List <MemberRole> memberRoles = memberRoleDAO.getAllByMemberId(id);
			if (memberRoles != null)
				for (MemberRole memberRole : memberRoles) {
					memberRoleDAO.delete(memberRole);
				}
			Member entity = memberDAO.get(id);
	...
	public boolean deleteForgotTemp(String code) {
		try {
			ForgotTemp entity = forgotTempDAO.get(code);
			if (entity != null) {
				forgotTempDAO.delete(entity);
			}
			return true;
		} catch (Exception e) {
			//e.printStackTrace();
	 ...
	 * @return 忘記密碼驗證資料
	 */
	public VerifyEmail getVerifyEmail(String email) {
		return verifyEmailDAO.get(email);
	}
	 ...
	 * @return 電子郵件驗證碼資料
	 */
	public VerifyEmail updateVerifyEmail(String email, String code) {
		try {
			VerifyEmail entity = verifyEmailDAO.get(email);
			if (entity != null) {
				verifyEmailDAO.delete(entity);
			}
			entity = new VerifyEmail();
			entity.setEmail(email);
			entity.setCode(code);
			verifyEmailDAO.insert(entity);
			return entity;
		} catch (Exception e) {
			//e.printStackTrace();
			return null;
	...
	public List<Member> getByOrgIds(Long[] orgIds) {
		return memberDAO.getByOrgIds(orgIds);
	}
