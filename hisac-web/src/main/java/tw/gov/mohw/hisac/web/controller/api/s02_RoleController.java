...
public class s02_RoleController extends BaseController {
	 ...
	 * @return 依照條件查詢角色資料
	 */
	@RequestMapping(value = "/s02/query", method = RequestMethod.POST)
	 ...
	 * @return 取得角色資料
	 */
	@RequestMapping(value = "/s02/query/id", method = RequestMethod.POST)
	public String QueryById(Locale locale, HttpServletRequest request, Model model, @RequestBody String json) {
		...
		if (menuService.getReadPermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			...
			sn_json.put("ModifyTime", role.getModifyTime());
			sn_array.put(sn_json);
			systemLogService.insert(baseControllerName, baseActionName, String.valueOf(id), SystemLogVariable.Action.Read, SystemLogVariable.Status.Success, getBaseIpAddress(), getBaseMemberAccount());
		} else {
			systemLogService.insert(baseControllerName, baseActionName, json, SystemLogVariable.Action.Read, SystemLogVariable.Status.Deny, getBaseIpAddress(), getBaseMemberAccount());
		}
	 ...
	 * @return 是否新增成功
	 */
	@RequestMapping(value = "/s02/create", method = RequestMethod.POST)
	public @ResponseBody String Create(Locale locale, HttpServletRequest request, @RequestBody String json) {
		...
		if (menuService.getCreatePermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			...
			if (roleService.isNameExist(name)) {
				...
				Role role = roleService.insertData(getBaseMemberId(), json);
				if (role != null) {
	 ...
	 * @return 是否更新成功
	 */
	@RequestMapping(value = "/s02/update", method = RequestMethod.POST)
	public @ResponseBody String Update(Locale locale, HttpServletRequest request, @RequestBody String json) {
		...
		if (menuService.getUpdatePermission(getBaseMemberId(), targetControllerName, targetActionName)) {
			...
			if (!roleService.isExist(id)) {
				...
				Role role = roleService.updateData(getBaseMemberId(), json);
				if (role != null) {
