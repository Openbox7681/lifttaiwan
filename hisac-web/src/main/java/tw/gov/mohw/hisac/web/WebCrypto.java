...
public class WebCrypto {
	 ...
	 */
	public static <K, V> void saveMap(Map<K, V> map, K key, V value) {
	...
	/**
	 * Clone the new safe object from the original object.
	 * 
	 * @param original
	 *            An original object
	 * @return A new object which is safe to use.
	 */
	public static <T> T getSafe(T original) {
		if (null == original) {
			return null;
		} else {
			Map<String, T> m = new HashMap<>();
			String code = UUID.randomUUID().toString();
			saveMap(m, code, original);
			return m.get(code);
		}
	}
}
