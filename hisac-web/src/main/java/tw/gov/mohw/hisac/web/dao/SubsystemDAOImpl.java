...
@Repository
@Transactional
public class SubsystemDAOImpl extends BaseSessionFactory implements SubsystemDAO {
	...
	public Subsystem get(Long id) {
		return getSessionFactory().getCurrentSession().get(Subsystem.class, id);
	}

	@SuppressWarnings({"deprecation", "unchecked"})
	public List<Subsystem> getAll() {
		Criteria cr = getSessionFactory().getCurrentSession().createCriteria(Subsystem.class);
		List<Subsystem> list = cr.list();
		if (list.size() > 0) {
			return list;
		} else {
			return null;
	...
	public Subsystem getByCode(String code) {
		...
		return (Subsystem) cr.uniqueResult();
	}
