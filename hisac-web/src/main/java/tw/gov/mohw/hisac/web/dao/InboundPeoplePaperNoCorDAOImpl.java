...
@Repository
@Transactional
public class InboundPeoplePaperNoCorDAOImpl extends BaseSessionFactory implements InboundPeoplePaperNoCorDAO {
	...
	public ViewInboundPeoplePaperNoCor get(Long id) {
		return getSessionFactory().getCurrentSession().get(ViewInboundPeoplePaperNoCor.class, id);
	}

	@SuppressWarnings({"deprecation", "unchecked"})
	public List<ViewInboundPeoplePaperNoCor> getAll() {
		Criteria cr = getSessionFactory().getCurrentSession().createCriteria(ViewInboundPeoplePaperNoCor.class);
		List<ViewInboundPeoplePaperNoCor> list = cr.list();
		if (list.size() > 0) {
			return list;
		} else {
			return null;
	...
	public ViewInboundPeoplePaperNoCor getByCode(String code) {
		...
		return (ViewInboundPeoplePaperNoCor) cr.uniqueResult();
	}
