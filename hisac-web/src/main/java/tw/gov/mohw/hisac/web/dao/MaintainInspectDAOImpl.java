...
@Repository
@Transactional
public class MaintainInspectDAOImpl extends BaseSessionFactory implements MaintainInspectDAO {
	...
	public MaintainInspect get(Long id) {
		return getSessionFactory().getCurrentSession().get(MaintainInspect.class, id);
	}
	...
	public List<MaintainInspect> getList(JSONObject obj){
		...
		Criteria cr = getSessionFactory().getCurrentSession().createCriteria(MaintainInspect.class);
		...
		List<MaintainInspect> list = cr.list();
		if (list.size() > 0) {
			return list;
		} else {
			return null;
