<%@ page language="java" pageEncoding="UTF8"%>
<%@ taglib prefix="s" uri="http://www.springframework.org/tags"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<footer id="footer">
	<div class="container">
        <div class="row">
          <div class="col-lg-3 col-md-4 middlecenter">
            <div class="footer_logo"><img src="<c:out value="${pageContext.request.contextPath}" />/resources/img/logo_most.svg"></div>
          </div>
          <div class="col-lg-9 col-md-8">
            <div class="footer_content" id="acolor2">
              <p>計畫委辦單位：科技部</p>
              <p>執行單位：財團法人國家實驗研究院科技政策研究與資訊中心</p>
              <p>地址：10636 臺北市和平東路二段106號1、14、15樓</p>
              <p>業務聯絡：(02)2737-7426/7746；系統聯絡：(02)2737-7145/7670</p>
              <p>E-mail:lift@narlabs.org.tw</p>
              <a href="<c:out value="${pageContext.request.contextPath}" />/privacy"> 
              	<span>隱私權宣告
              	</span>
              </a>
              <span>｜ </span>
              <a href="<c:out value="${pageContext.request.contextPath}" />/copyright"> 
              	<span>版權宣告</span>
              </a>
            </div>
            <div class="divline"></div>
            <div class="footer_content">
              <p>版權所有©2019科技部科教發展及國際合作司</p>
              <p>Department of International Cooperation and Science Education</p>
              <p>本網站設計支援Edge、Firefox及Chrome，網頁設計最佳瀏覽解析度為1280x768以上</p>
            </div>
          </div>
        </div>
      </div>
      
      <div id="cookieConsent">
      
      {{cookieData}}
       
      
      <button class="cookieAccept">同意</button></div>
      
	
</footer>
