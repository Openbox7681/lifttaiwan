...
<script src="<c:out value="${pageContext.request.contextPath}" />/resources/js/pub/p06.js"></script>
	...
	<div class="container">
					...
					<table
						...
						<tbody>
							<tr ng-repeat="item in allitems">
								...
								<td><a href="#" ng-click="queryDetailData(item.Id)">{{item.Cve}}{{item.Title}}</a></td>
							</tr>
						</tbody>
					</table>
				</div>
		...
		<div id="divDetail" style="display: none" ng-show="isDetail">
			<div class="row detail_block">
				...
				<h3 class="detailTitle">({{detailIncidentId}}){{detailTitle}}</h3>
				<h5 class="detailSource">
					<span><s:message code="newsSoruce" />&nbsp;:&nbsp;{{detailReporterName}}</span>
				...
				<div class="detailIncidentDiscoveryTime"
					...
					<div>
						<s:message code="p06detailIncidentDiscoveryTime" />
						：{{detailIncidentDiscoveryTime}}
					</div>
				</div>
				<div class="detailContent">
					<div class="well well-sm" ng-if="!isBreakLine" ng-bind-html="detailContent"></div>
					<div class="well well-sm" ng-if="isBreakLine" ng-bind-html="detailContent | trustHtml"></div>
				</div>
				<div class="detailResponderPartyName"
					ng-show="detailResponderPartyName">
					<div>
						<s:message code="p06detailResponderPartyName" />
				...
				<div class="detailResponderContactNumbers"
					...
					<div>
						<s:message code="p06detailResponderContactNumbers" />
				...
				<div class="detailResponderElectronicAddressIdentifiers"
					...
					<div>
						<s:message code="p06detailResponderElectronicAddressIdentifiers" />
				...
				<div class="detailImpactQualification"
					...
					<div>
						<s:message code="p06detailImpactQualification" />
						：{{detailImpactQualification}}
					</div>
				</div>
				<div class="detailConfidence" ng-show="detailConfidence">
					<span><s:message code="p06detailConfidence" />：{{detailConfidence}}</span>
				</div>
				<div class="detailReference" ng-show="detailReference">
					<span><s:message code="p06detailReference" />：<div ng-repeat="_detailReference in detailReferences"><a ng-show="_detailReference.trim().length > 0"
						ng-href="{{_detailReference}}" target="_secbuzzer"><i class="fa fa-external-link-alt"></i>&nbsp;{{_detailReference}}</a></div></span>
				</div>
				<div class="detailAffectedSoftwareDescription"
					ng-show="detailAffectedSoftwareDescription">
					<span><s:message code="p06detailAffectedSoftwareDescription" />：{{detailAffectedSoftwareDescription}}</span>
				</div>
			</div>
			<div class="row detail_block" ng-show="itemAttachments.length > 0">
				...
				<ul class="detailAttachFile">
					<li ng-repeat="item in itemAttachments"><s:message
						...
						ng-href="<c:out value="${pageContext.request.contextPath}" />/pub/api/p06/attach/download/{{id}}/{{item.Id}}"
