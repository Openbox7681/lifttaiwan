...
<script
	src="<c:out value="${pageContext.request.contextPath}" />/resources/js/activity.js"></script>
<body class="no-skin" ng-controller="getAppController">
	<%@ include file="./include/head_navbar.jsp"%>
	<div class="container">
		<div class="row">
			<h4 class="form_heading form_heading_fix form_heading_orange">
				<img
			...
			<div class="row">
				<div class="table-responsive">
					<table
						...
						<thead>
							<tr>
								<th class="col-xs-2"><s:message code="activityPostDateTime" /></th>
								<th class="col-xs-3"><s:message code="activityDateRange" /></th>
								<th><s:message code="activityTitle" /></th>
							</tr>
						</thead>
						<tbody>
							<tr ng-repeat="item in allitems">
								<td class="text-center">{{item.Date}}</td>
								<td class="text-center">{{item.StartDateTime}}<s:message
										code="globalDataFromTo" />{{item.EndDateTime}}
								</td>
		...
		<div id="divDetail" style="display: none">
			<div class="row detail_block">
				...
				<h5 class="detailSource">
					<span><s:message code="activitySoruce" />&nbsp;:&nbsp;{{detailSourceName}}</span>
					<span class="pull-right"><s:message
							code="activityPostDateTime" />&nbsp;:&nbsp;{{detailPostDataTime}}</span>
				</h5>
				...
				<h5 class="detailSource" ng-if="detailSourceLink != null && detailSourceLink != ''">
					<span><s:message code="s32SourceLink" />&nbsp;:&nbsp;</span><a ng-href="{{detailSourceLink}}" target="_news">{{detailSourceLink}}</a>
				</h5>
			</div>
			<div class="row detail_block" ng-show="itemAttachments.length > 0">
				...
				<ul class="detailAttachFile">
					<li ng-repeat="item in itemAttachments"><s:message
						...
						ng-href="<c:out value="${pageContext.request.contextPath}" />/public/api/activity/attach/download/{{id}}/{{item.Id}}"
			...
			<div class="row detail_block">
				<a class="btn btn-default btn_custom"
					ng-href="<c:out value="${pageContext.request.contextPath}" />/"> <i
