...
function getAppController($rootScope, $scope, $http, $cookieStore, $anchorScroll, $location) {
	...
	$scope.queryData = function(page) {
		...
		$http.post('./common/i02/query', request, csrf_config).then(function(response) {
	...
	$scope.editData = function(id) {
		...
		$http.post('./common/i02/query/id', request, csrf_config).then(function(response) {
			$scope.openEdit();
			$scope.btnIns = false;
			$scope.btnUpd = true;
		...
		bootbox.confirm({
			...
			callback: function(result) {
				if (result) {
					...
					$http.post('./common/i02/delete', request, csrf_config).then(function(response) {
