...
function getAppController($rootScope, $scope, $http, $cookieStore,
	...
	$scope.queryData = function(page) {
		...
		$http.post('./api/m02/query', request, csrf_config).then(function(response) {
	...
	$scope.editData = function(id) {
		...
		var request = {Id:id};
		$http.post('./api/m02/query/id', request, csrf_config).then(function(response) {
		...
		bootbox.confirm({
			...
			callback: function(result) {
				if (result) {
					...
					$http.post('./api/m02/delete/', request, csrf_config).then(function(response) {
	...
	$scope.createData = function() {
			...
			$http.post('./api/m02/create/', request, csrf_config).then(
					function(response) {
						if (response.data.success) {
							$scope.queryData($scope.currentPage);
							bootbox.alert({
	...
	$scope.updateData = function() {
			...
			$http.post('./api/m02/update/', request, csrf_config).then(
					function(response) {
						if (response.data.success) {
							$scope.queryData($scope.currentPage);
							bootbox.alert({
