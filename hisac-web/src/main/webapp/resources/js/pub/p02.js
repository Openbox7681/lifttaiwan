...
function getAppController($scope, $rootScope, $http, $window) {
	...
	$scope.queryData = function(page) {
		...
		$http.post('./api/p02/query', request, csrf_config).then(function(response) {
	...
	$scope.queryDetailData = function(id) {
		...
		$http.post('./api/p02/query/id', request, csrf_config).then(function(response) {
	...
	$scope.insertStarRating = function(){
		...
		$http.post('./api/p02/starRating/insert', request, csrf_config).then(
			function(response){
	...
	$scope.insertComment = function(){
		...
		$http.post('./api/p02/comment/insert', request, csrf_config).then(
			function(response){
				if (response.data.success){
					$scope.queryDetailData($scope.id);
				}else{
	...
	$scope.CancelReply = function(id){
		...
		$http.post('./api/p02/comment/delete', request, csrf_config).then(
				function(response){
					if (response.data.success){
						$scope.queryDetailData($scope.id);
					}else{
	...
	$scope.queryDetailAttachData = function(id) {
		...
		$http.post('./api/p02/attach/query', request, csrf_config).then(function(response) {			
