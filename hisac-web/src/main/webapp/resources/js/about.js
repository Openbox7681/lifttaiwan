...
function getAppController($scope, $http, $window) {
	$scope.queryContent = function() {
		...
		$http.post('./public/api/about/query/id', request, csrf_config).then(function(response) {
	...
	$scope.queryDetailAttachData = function(id) {
		...
		$http.post('./public/api/about/attach/query', request, csrf_config2).then(function(response) {			
