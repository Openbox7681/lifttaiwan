...
function getAppController($rootScope, $scope, $http, $cookieStore, Upload) {
	...
	$scope.queryData = function(page) {
		...
		if ($scope.QueryNewsManagementGroupId == "")
			$scope.QueryNewsManagementGroupId = null;
		...
		var request = {
			...
			NewsManagementGroupId : $scope.QueryNewsManagementGroupId,
		...
		$http.post('./api/s31/query', request, csrf_config).then(function(response) {
		...
		bootbox.confirm(
			...
			function(result) {
				if (result) {
					...
					$http
						.post('./api/s31/examine', request, csrf_config)
						.then(
							function(response) {
								...
								bootbox.dialog({
									message : "<span class='bigger-110'>最新消息" + action + "失敗</span>",
									buttons : {
										"success" : {
											"label" : "<i class='ace-icon fa fa-close'></i> 關閉",
											"className": 'btn-danger',
		...
		bootbox.confirm({
		    ...
		    callback: function (result) {
		        if (result) {
		        	...
		        	$http.post('./api/s31/disable', request, csrf_config)
					.then(function() {
						$scope.queryData($scope.currentPage);
					}).finally(function() {
						bootbox.hideAll();;
...
// +$scope.IsBreakLine);
	...
	$scope.updateData = function(exit,check) {	
		...
		if (!$scope.editForm.$valid && check) {
			...
			$http.post('./api/s31/update', request, csrf_config).then(
				function(response) {
					if (response.data.success) {
			...
			bootbox.confirm({
				...
				callback: function(result) {
					if (result) {
						...
						$http.post('./api/s31/delete', request, csrf_config).then(function(response) {
							if (response.data.success) {
								bootbox.alert({
									message : response.data.msg,
									buttons : {
		...
		// Get NewsManagementGroup Start
		$scope.getNewsManagementGroup = function() {
			var request = {};
			$http.post('./api/s31/query/newsManagementGroup', request, csrf_config).then(function(response) {
				$scope.newsManagementGroups = response.data;
			}).catch(function() {
				bootbox.alert({
					message : globalReadDataFail,
					buttons : {
		...
		$scope.getNewsManagementGroup();
		// Get NewsManagementGroup End
		...
		$scope.queryImageData = function(id) {
			...
			$http.post('./api/s31/pic/query', request, csrf_config).then(function(response) {
			...
			bootbox.confirm({
				...
				callback: function(result) {
					if (result) {
						...
						$http.post('./api/s31/pic/delete', request, csrf_config).then(function(response) {
							if (response.data.success) {
								$scope.queryImageData($scope.Id);
								bootbox.alert({
									message : response.data.msg,
		...
		$scope.queryAttachmentData = function(id) {
			...
			$http.post('./api/s31/attach/query', request, csrf_config).then(function(response) {
			...
			bootbox.confirm({
				...
				callback: function(result) {
					if (result) {
						...
						$http.post('./api/s31/attach/delete', request, csrf_config).then(function(response) {
							if (response.data.success) {
								$scope.queryAttachmentData($scope.Id);
								bootbox.alert({
									message : response.data.msg,
		...
		$scope.queryButtonCount = function() {
			...
			$http.post('./api/s31/query/button/count', request, csrf_config).then(function(response) {
