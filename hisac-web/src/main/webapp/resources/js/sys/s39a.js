...
function getAppController($rootScope, $scope, $http, $cookieStore, Upload) {
	...
	$scope.maxPages = 0;
	$scope.total = 0;
	$scope.sorttype = "IncidentDiscoveryTime";
...
// + $scope.btnUpd + "；$scope.Action = " + $scope.Action);
	...
	$scope.updateData = function(exit,check) {	
		if (!$scope.editForm.$valid && check) {
			...
			$http.post('./api/s39/modify', request, csrf_config).then(
				function(response) {
					if (response.data.success) {
	...
	//  轉勒索專區 Start
	$scope.toMalware = function(id){
		bootbox.confirm({
			message: globalSureToMalware ,
			buttons: {
				confirm: {
		...
		callback: function(result){
			...
			if (result) {
				...
				$http.post('./api/s39/createMalware/id', request, csrf_config).then(function(response) {
