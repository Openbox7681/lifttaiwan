...
function getAppController($rootScope, $scope, $http, $cookieStore) {
	...
	$scope.queryOrgReport = function (){
		...
		$http.post('./api/s43/orgReportResult', request, csrf_config).then(function(data) {
			//登入次數統計-登入次數前十名
	...
	$scope.queryMemberReport = function (){
		$scope.query = true;
		if (startDate == "")
			startDate = null;
        if (endDate == "")
		...
		$http.post('./api/s43/memberReportResult', request, csrf_config).then(function(data) {
			//會員統計
	...
	$scope.queryData = function() {	
		$scope.queryMemberReport();
		$scope.queryOrgReport();
...
//				QueryEdate : endDate
//			};
//		$http.post('./api/s43/memberReport', request, csrf_config).then(function(data) {
//			//會員統計
//			$scope.MemberCount = data.data.MemberCount
//			$scope.CIMemberCount = data.data.CIMemberCount
//			$scope.NonCIAdvancedCount = data.data.NonCIAdvancedCount
