...
function getAppController($rootScope, $scope, $http, $cookieStore, Upload) {
	...
	$scope.queryData = function(page) {
		...
		$http.post('./api/s45/query', request, csrf_config).then(function(response) {
			...
			function(result) {
				if (result) {
					...
					var request = {
						...
						Status : status,
						PreStatus : preStatus,
						TableName : "malware_management",
						Pre : "HISAC",
						Opinion : $scope.Opinion
					};
					
					$http
						.post('./api/s45/examine', request, csrf_config)
						.then(
							function(response) {
		...
		bootbox.confirm({
		    ...
		    callback: function (result) {
		        if (result) {
		        	...
		        	$http.post('./api/s45/disable', request, csrf_config)
					.then(function() {
						$scope.queryData($scope.currentPage);
					}).finally(function() {
						bootbox.hideAll();;
...
//		console.log("s32.js → editData() → $scope.btnIns=" + $scope.btnIns + "；$scope.btnUpd=" + $scope.btnUpd + "；$scope.Action = " + $scope.Action);
		...
		$http.post('./api/s45/query/id', request, csrf_config).then(function(response) {
...
//			console.log("s32.js → editData() → $scope.btnIns=" + $scope.btnIns + "；$scope.btnUpd=" + $scope.btnUpd + "；$scope.Action = " + $scope.Action);
	...
	$scope.createData = function(exit, check) {
		...
		if (!$scope.editForm.$valid && check) {
			...
			$http.post('./api/s45/create', request, csrf_config).then(
				function(response) {
					if (response.data.success) {
	...
	$scope.updateData = function(exit,check) {	
		...
		if (!$scope.editForm.$valid && check) {
			...
			$http.post('./api/s45/update', request, csrf_config).then(
				function(response) {
					if (response.data.success) {
		...
		bootbox.confirm({
			...
			callback: function(result) {
				if (result) {
					...
					$http.post('./api/s45/delete', request, csrf_config).then(function(response) {
						if (response.data.success) {
							bootbox.alert({
								message : response.data.msg,
								buttons : {
	...
	$scope.queryImageData = function(id) {
		var request = {
			MalwareManagementId : id
		};
		$http.post('./api/s45/pic/query', request, csrf_config).then(function(response) {
		...
		bootbox.confirm({
			...
			callback: function(result) {
				if (result) {
					var request = {
						Id : id,
						MalwareManagementId : $scope.Id
					};
					$http.post('./api/s45/pic/delete', request, csrf_config).then(function(response) {
						if (response.data.success) {
							$scope.queryImageData($scope.Id);
							bootbox.alert({
								message : response.data.msg,
	...
	$scope.queryAttachmentData = function(id) {
		...
		$http.post('./api/s45/attach/query', request, csrf_config).then(function(response) {
		...
		bootbox.confirm({
			...
			callback: function(result) {
				if (result) {
					...
					$http.post('./api/s45/attach/delete', request, csrf_config).then(function(response) {
						if (response.data.success) {
							$scope.queryAttachmentData($scope.Id);
							bootbox.alert({
								message : response.data.msg,
	...
	$scope.queryButtonCount = function() {
		...
		$http.post('./api/s45/query/button/count', request, csrf_config).then(function(response) {
