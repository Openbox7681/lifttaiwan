...
function getAppController($rootScope, $scope, $http, $cookieStore, Upload) {
	...
	$scope.queryData = function(page) {
		...
		$http.post('./api/s40/query', request, csrf_config).then(function(response) {
			...
			function(result) {
				if (result) {
					...
					var request = {
						...
						Status : status,
						PreStatus : preStatus,
						TableName : "weakness_management",
						Pre : "HISAC",
						Opinion : $scope.Opinion
					};
					
					$http
						.post('./api/s40/examine', request, csrf_config)
						.then(
							function(response) {
								...
								bootbox.dialog({
									message : "<span class='bigger-110'>軟體弱洞漏洞通告" + action + "失敗</span>",
									buttons : {
										"success" : {
											"label" : "<i class='ace-icon fa fa-close'></i> 關閉",
											"className": 'btn-danger',
...
//		console.log("editData() → $scope.btnIns=" + $scope.btnIns + "；$scope.btnUpd=" + $scope.btnUpd + "；$scope.Action = " + $scope.Action);
		...
		$http.post('./api/s40/query/id', request, csrf_config).then(function(response) {
			$scope.openEdit();
...
//			console.log("editData() → $scope.btnIns=" + $scope.btnIns + "；$scope.btnUpd=" + $scope.btnUpd + "；$scope.Action = " + $scope.Action);
	...
	$scope.createData = function(exit, check) {
		...
		if (!$scope.editForm.$valid && check) {
			...
			$http.post('./api/s40/create', request, csrf_config).then(
				function(response) {
					if (response.data.success) {
	...
	$scope.updateData = function(exit,check) {	
		...
		if (!$scope.editForm.$valid && check) {
			...
			$http.post('./api/s40/update', request, csrf_config).then(
				function(response) {
					if (response.data.success) {
		...
		bootbox.confirm({
			...
			callback: function(result) {
				if (result) {
					...
					$http.post('./api/s40/delete', request, csrf_config).then(function(response) {
						if (response.data.success) {
							bootbox.alert({
								message : response.data.msg,
								buttons : {
	...
	$scope.queryAttachmentData = function(id) {
		...
		$http.post('./api/s40/attach/query', request, csrf_config).then(function(response) {
		...
		bootbox.confirm({
			...
			callback: function(result) {
				if (result) {
					var request = {
						Id : id,
						WeaknessManagementId : $scope.Id
					};
					$http.post('./api/s40/attach/delete', request, csrf_config).then(function(response) {
						if (response.data.success) {
							$scope.queryAttachmentData($scope.Id);
							bootbox.alert({
								message : response.data.msg,
	...
	$scope.queryButtonCount = function() {
		...
		$http.post('./api/s40/query/button/count', request, csrf_config).then(function(response) {
