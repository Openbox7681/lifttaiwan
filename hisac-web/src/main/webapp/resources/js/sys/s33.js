...
function getAppController($rootScope, $scope, $http, $cookieStore, $anchorScroll, $location) {
	...
	$scope.queryData = function(page) {
		...
		$http.post('./api/s33/query', request, csrf_config).then(function(response) {
	...
	$scope.editData = function(id) {
		...
		var request = {Id:id};
		$http.post('./api/s33/query/id', request, csrf_config).then(function(response) {
			//console.log("Id="+response.data[0].IsEnable);
			$scope.openEdit();
			$scope.btnIns = false;
			$scope.btnUpd = true;
	...
	$scope.createData = function() {
		...
		$http.post('./api/s33/create', request, csrf_config).then(
				function(response) {
	...
	$scope.updateData = function() {
		var request = {
			...
			$http.post('./api/s33/update', request, csrf_config).then(
				function(response) {
					if (response.data.success) {
						$scope.queryData($scope.currentPage);
						bootbox.alert({
				...
				bootbox.confirm({
					...
					callback: function(result) {
						if (result) {
							...
							$http.post('./api/s33/delete', request, csrf_config).then(function(response) {
								if (response.data.success) {
									bootbox.alert({
										message : response.data.msg,
										buttons : {
