...
function getAppController($rootScope, $scope, $http, $cookieStore, $anchorScroll, $location) {
	...
	$scope.queryData = function(page) {
		...
		$http.post('./api/s04/query', request, csrf_config).then(function(response) {
	...
	$scope.editData = function(id) {
		...
		$http.post('./api/s04/query/id', request, csrf_config).then(function(response) {
			$scope.openEdit();
			$scope.btnIns = false;
			$scope.btnUpd = true;
	...
	$scope.createData = function() {
		...
		$http.post('./api/s04/create', request, csrf_config).then(
			function(response) {
	...
	$scope.updateData = function() {
		...
		$http.post('./api/s04/update', request, csrf_config).then(
			function(response) {
				if (response.data.success) {
					$scope.queryData($scope.currentPage);
					bootbox.alert({
			...
			bootbox.confirm({
				...
				callback: function(result) {
					if (result) {
						...
						$http.post('./api/s04/delete', request, csrf_config).then(function(response) {
							if (response.data.success) {
								bootbox.alert({
									message : response.data.msg,
									buttons : {
