...
function getAppController($rootScope, $scope, $http, $cookieStore,
	...
	$scope.queryYearData = function() {
		...
		$http.post('./api/s47/year/query', request, csrf_config).then(function(response) {
	...
	$scope.queryData = function(page) {
		...
		//console.log("request="+JSON.stringify(request));
		$http.post('./api/s47/query', request, csrf_config).then(function(response) {
			
			//console.log("datatable="+JSON.stringify(response.data.datatable));
			$scope.OrgCiLevelList = response.data.datatable;
	...
	$scope.updateData = function() {
		...
		if($scope.QueryOrgId!=null)
			...
			var request = {
				Year :Year,	
				OrgCiLevelList : $scope.OrgCiLevelList
			};
			
			console.log("111request="+JSON.stringify(request));
			$http
					.post('./api/s47/update', request, csrf_config)
					.then(
							function(response) {
